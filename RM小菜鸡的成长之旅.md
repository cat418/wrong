...
是ROS2中的一个方法，主要用于在节点中声明参数，允许为节点注册一个参数，并指定改参数的名称、类型及可选的默认值，在ROS2中，参数是一种动态配置的机制，允许用户在节点运行时改变其行为。

### **2.TF2**

#:机器人系统中可能有多个坐标系，比如 **"map"**、**"odom"**、**"base_link"** 和 **"camera_link"**。

`tf2` 用来管理这些坐标系之间的关系和转换。

### **3.坐标系**

**imu(惯性测量单元)：**是一种能够测量物体在三维空间中运动状态的传感器。

**惯性坐标系: **是imu第一次通电时的坐标系。(是固定的一个抽象出来的坐标系)

**imu_frame（imu坐标系）:**小车上imu和装甲板在一条直线上,朝向相同,imu坐标系会和装甲板一起旋转。

要实现**camera_frame->imu_frame->惯性系** 的转换
...
**ResNet:**用语图像分类和目标检测，特点是解决了深层网络的梯度消失问题

**EfficienTNET:**更高效的利用计算资源，适合在资源受限的环境中部署

**Faster R-CNN,YOLO,YOLOX:**专注于目标检测，YOLO系列因其速度快而被广泛使用
...
**BERT:**擅长文本理解任务，例如问答系统和情感分析

**GPT系列：**专注于文本生成任务，泪如内容创作和代码生成

**Vision Transformer(ViT):**将Transformer用于图像分类

==**3.循环神经网络(RNN)**==

**LSTM和GRU:**解决长序列任务中的梯度问题，常用于处理时间序列数据

==**4.生成对抗网络(GAN)**==

**DCGAN**: 基础的图像生成网络。

**StyleGAN**: 生成高质量图像，用于艺术设计或人脸生成。

**CycleGAN**: 图像风格迁移，例如将普通照片转为艺术画风。

==**5.自编码器**==

**Variational Autoencoder (VAE)**: 生成模型，用于数据建模。
